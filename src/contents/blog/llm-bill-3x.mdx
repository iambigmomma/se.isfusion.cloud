...
> **A note on pricing:** All token prices in this article were correct at the time of writing (June 2026), but LLM pricing changes frequently — providers cut rates, add tiers, and rename models often, so the numbers may already be out of date by the time you read this. Treat every figure here as illustrative of the _pattern_, not a live quote; always check the provider's official pricing page before budgeting.
...
The biggest single lever is routing simple requests to smaller, cheaper models. Claude Haiku 4.5 costs $1.00/M input; Claude Sonnet 4.6 costs $3.00/M — a 3× price difference, and if a capable open-weight model clears your accuracy floor, the gap widens to 10×+. For tasks where a smaller model produces equivalent quality (classification, summarization, FAQ responses from retrieved context), running Sonnet is waste.
