...
- A [GPU Droplet](https://www.digitalocean.com/products/gpu-droplets).
...
Stable Diffusion can run on a CPU, but it's slow. Running it on a GPU drastically improves the performance. With DigitalOcean's offerings, you don't need the latest H100 GPU to see improvements—though that's an option if you want top-tier performance. You can easily compare the differences between CPU and GPU rendering by spinning up a GPU droplet.
...
4. In the search bar, type **"CivitAI Browser+"** and click the **Install** button.
   ![extensions](https://res.cloudinary.com/iambigmomma/image/upload/v1685713949/blog/stable-diffusion-gpu-droplet/extensions.png)
